...
import com.tu.pp.service.UserSubscriptionQueryService;
import com.tu.pp.service.UserSubscriptionService;
import com.tu.pp.service.criteria.UserSubscriptionCriteria;
import com.tu.pp.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.tu.pp.domain.UserSubscription}.
 */
@RestController
@RequestMapping("/api")
public class UserSubscriptionResource {
    ...
    public ResponseEntity<UserSubscription> createUserSubscription(@Valid @RequestBody UserSubscription userSubscription)
        ...
        UserSubscription result = userSubscriptionService.save(userSubscription);
    ...
    public ResponseEntity<UserSubscription> updateUserSubscription(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, userSubscription.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<UserSubscription> partialUpdateUserSubscription(
        ...
        Optional<UserSubscription> result = userSubscriptionService.partialUpdate(userSubscription);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, userSubscription.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<UserSubscription>> getAllUserSubscriptions(UserSubscriptionCriteria criteria, Pageable pageable) {
        log.debug("REST request to get UserSubscriptions by criteria: {}", criteria);
        Page<UserSubscription> page = userSubscriptionQueryService.findByCriteria(criteria, pageable);
    ...
    public ResponseEntity<UserSubscription> getUserSubscription(@PathVariable Long id) {
        ...
        Optional<UserSubscription> userSubscription = userSubscriptionService.findOne(id);
